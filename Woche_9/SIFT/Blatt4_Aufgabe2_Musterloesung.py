...
# custom imports
from SIFT_KeyPoint import KeyPoint # represents a keypoint or extremum found
...
def create_scale_space(u_in: NDArray[np.float32], sift_params: SIFT_Params) -> Tuple[list[list[NDArray[np.float32]]],list[float],list[list[float]]]:
    ...
    u: NDArray[np.float32] = cv.resize(u_in, (0,0), fx=1.0 / sift_params.delta_min, fy=1.0 / sift_params.delta_min, interpolation=cv.INTER_LINEAR)
    ...
    v_1_0: NDArray[np.float32] = cv.GaussianBlur(u, (0,0), sigma_extra)
    first_octave: list[NDArray[np.float32]] = [v_1_0]
    ...
    for o in range(1, sift_params.n_oct):
        ...
        seed: NDArray[np.float32] = scale_space[o-1][sift_params.n_spo-1]
        ...
        current_octave: list[NDArray[np.float32]] = [seed]
...
def create_dogs(scale_space: list[list[NDArray[np.float32]]], sift_params: SIFT_Params) -> list[list[NDArray[np.float32]]]:
    ...
    """
    dogs: list[list[NDArray[np.float32]]] = []
    # for each octave
    for o in range(0, sift_params.n_oct):
        octave: list[NDArray[np.float32]] = scale_space[o]
        dog_row: list[NDArray[np.float32]] = []
...
def find_discrete_extremas(dogs: list[list[NDArray[np.float32]]], sift_params: SIFT_Params, sigmas: list[list[float]], deltas: list[float]) -> list[KeyPoint]:
    ...
    for o in range(0, sift_params.n_oct):
        ...
        octave: list[NDArray[np.float32]] = dogs[o]
        # for each dog image in the octave excluding first and last image
        for s in range(1, sift_params.n_spo+1):
            # the current dog image
            current: NDArray[np.float32] = octave[s]
            # the one before
            before: NDArray[np.float32] = octave[s-1]
            # the one after
            after: NDArray[np.float32] = octave[s+1]
...
def taylor_expansion(extremas: list[KeyPoint],
    ...
    for extremum in extremas:
        ...
        for _ in range(5):
            ...
            current: NDArray[np.float32] = dog_scales[extremum.o][s]
            previous: NDArray[np.float32] = dog_scales[extremum.o][s-1]
            next_scale: NDArray[np.float32] = dog_scales[extremum.o][s+1]
...
def filter_extremas(extremas: list[KeyPoint], dogs: list[list[NDArray[np.float32]]], sift_params: SIFT_Params) -> list[KeyPoint]:
    ...
    """
    filtered_extremas: list[KeyPoint] = []
    for extremum in extremas:
        ...
        n: int = extremum.n # y-coordinate
        # current dog image
        current: NDArray[np.float32] = dogs[extremum.o][s]
...
def gradient_2d(scale_space: list[list[NDArray[np.float32]]], sift_params: SIFT_Params) -> dict[Tuple[int, int, int, int], Tuple[float, float]]:
    ...
    Args:
        scale_space (list[list[NDArray[np.float32]]]): the scale space to calculate the gradient from
        sift_params (SIFT_Params): the sift parameters
    ...
    for o in range(0, sift_params.n_oct):
        ...
        for s in range(0, sift_params.n_spo+3):
            # get current image
            v_o_s: NDArray[np.float32] = scale_space[o][s]
            # for each pixel in the image
...
def float_modulo(val: float, mod: float) -> float:
    ...
    for keypoint in keypoints:
        ...
        if(border_limit <= key_x and
           ...
           key_y <= image.shape[0]-border_limit):
            ...
            h_k: NDArray[np.float32] = np.zeros(sift_params.n_bins)
            ...
            for k in range(sift_params.n_bins):
                ...
                if(h_k[k] > prev_hist and h_k[k] > next_hist and h_k[k] >= sift_params.t*max_h_k):
                    ...
                    if(angle > 2*np.pi):
                        angle -= 2*np.pi
                    new_keypoint = KeyPoint(keypoint.o,
                                            keypoint.s,
                                            keypoint.m,
                                            keypoint.n,
                                            keypoint.sigma,
    ...
    for keypoint in keypoints:
        ...
        if(border_limit <= key_x and
           ...
           key_y <= image.shape[0]-border_limit):
            ...
            histograms: list[list[NDArray[np.float32]]] = [[np.zeros(sift_params.n_ori) for _ in range(sift_params.n_hist)] for _ in range(sift_params.n_hist)]
